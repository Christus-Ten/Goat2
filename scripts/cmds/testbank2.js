...
module.exports = {
	...
	onStart: async function ({ message, args, event, usersData, threadsData, getLang, api }) {
		...
		switch (command) {
			...
			// === GAMING & FUN ===
			case "gamble":
				return this.gamble(message, args, userData, usersData, bank, bankData, senderID, fonts);

			case "lottery":
				return this.lottery(message, args, userData, usersData, bank, bankData, senderID, fonts);

			case "slots":
				return this.slots(message, args, userData, usersData, bank, bankData, senderID, fonts);

			case "blackjack":
				return this.blackjack(message, args, userData, usersData, bank, bankData, senderID, fonts);

			case "roulette":
				return this.roulette(message, args, userData, usersData, bank, bankData, senderID, fonts);
			...
			case "leaderboard":
				return this.leaderboard(message, bankData, fonts, api, usersData);

			case "rob":
				return this.rob(message, args, bank, bankData, senderID, event, fonts, usersData);
...
🎲 bank gamble <amount> - High-risk, high-reward games (uses wallet)
🎫 bank lottery [buy/check] - Weekly lottery draws (uses wallet)
🎰 bank slots <amount> - Vegas-style slot machines (uses wallet)
🃏 bank blackjack <amount> - Classic card game (uses wallet)
🎯 bank roulette <amount> <bet> - European roulette (uses wallet)
...
`;
	...
	business: async function (message, args, bank, bankData, senderID, fonts) {
		...
		if (!action || action === "list") {
			...
			businessList += `• bank business upgrade <index>`;
		...
		if (action === "upgrade") {
			...
			if (isNaN(index) || index < 0 || index >= bank.businesses.length) {
				return message.reply(fonts.bold("❌ Invalid business index. Use 'bank business list' to see your businesses."));
			}

			const business = bank.businesses[index];
			const upgradeCost = this.marketData.businesses[business.type].cost * 0.5 * business.level; // 50% of original cost per level

			if (bank.balance < upgradeCost) {
				return message.reply(fonts.bold(`❌ Insufficient funds. Upgrade cost: $${upgradeCost.toLocaleString()}`));
			}

			bank.balance -= upgradeCost;
			business.level++;
			business.revenue = this.marketData.businesses[business.type].income * business.level;
			...
			await bankData.set(senderID, bank);
			return message.reply(fonts.bold(`✅ Upgraded ${business.name} to level ${business.level} for $${upgradeCost.toLocaleString()}!`));
		}
	},
	...
	realEstate: async function (message, args, bank, bankData, senderID, fonts) {
		...
		if (!action || action === "list") {
			...
			propertyList += `• bank property sell <index>`;
		...
		if (action === "sell") {
			const index = parseInt(args[2]) - 1;
			if (isNaN(index) || index < 0 || index >= bank.realEstate.length) {
				return message.reply(fonts.bold("❌ Invalid property index. Use 'bank property list' to see your properties."));
			}

			const property = bank.realEstate[index];
			const sellPrice = Math.floor(property.value * 0.9); // 10% depreciation on sale

			bank.balance += sellPrice;
			bank.realEstate.splice(index, 1);

			bank.transactions.push({
				type: "property_sale",
				amount: sellPrice,
				date: Date.now(),
				description: `Sold ${property.name}`
			});

			await bankData.set(senderID, bank);
			return message.reply(fonts.bold(`✅ Sold ${property.name} for $${sellPrice.toLocaleString()}!`));
		}
	},
	...
	luxury: async function (message, args, bank, bankData, senderID, fonts) {
		...
		if (!action || action === "list") {
			...
			luxuryList += `• bank luxury sell <index>`;
		...
		if (action === "sell") {
			const index = parseInt(args[2]) - 1;
			if (isNaN(index) || index < 0 || index >= bank.luxury.length) {
				return message.reply(fonts.bold("❌ Invalid luxury index. Use 'bank luxury list' to see your items."));
			}

			const item = bank.luxury[index];
			const sellPrice = Math.floor(item.value * 0.7); // 30% depreciation

			bank.balance += sellPrice;
			bank.luxury.splice(index, 1);
			...
			await bankData.set(senderID, bank);
			return message.reply(fonts.bold(`✅ Sold ${item.name} for $${sellPrice.toLocaleString()}!`));
		}
	},
	...
	buyCar: async function (message, args, bank, bankData, senderID, fonts) {
		...
		if (action === "sell") {
			const index = parseInt(args[2]) - 1;
			if (isNaN(index) || index < 0 || index >= bank.vehicles.length) {
				return message.reply(fonts.bold("❌ Invalid vehicle index. Use 'bank car list' to see your vehicles."));
			}

			const vehicle = bank.vehicles[index];
			const sellPrice = Math.floor(vehicle.currentValue * 0.8); // 20% depreciation on sale

			bank.balance += sellPrice;
			bank.vehicles.splice(index, 1);

			bank.transactions.push({
				type: "vehicle_sale",
				amount: sellPrice,
				date: Date.now(),
				description: `Sold ${vehicle.name}`
			});

			await bankData.set(senderID, bank);
			return message.reply(fonts.bold(`✅ Sold ${vehicle.name} for $${sellPrice.toLocaleString()}!`));
		}
	},

	// Gaming functions (uses wallet)
	gamble: async function (message, args, userData, usersData, bank, bankData, senderID, fonts) {
		const amount = parseInt(args[1]);
		if (!amount || amount <= 0) {
			return message.reply(fonts.bold(`
🎰 GAMBLING GAMES
...
Available Games:
• bank gamble <amount> - Classic risk/reward (uses wallet)
• bank slots <amount> - Slot machine (uses wallet)
• bank blackjack <amount> - Card game (uses wallet)
• bank roulette <amount> <bet> - Roulette wheel (uses wallet)

Your Wallet: $${userData.money.toLocaleString()}
Gambling Skill: ${bank.skills.gambling}
			`));
		}

		if (userData.money < amount) {
			return message.reply(fonts.bold("❌ Insufficient funds in your wallet."));
		}
		...
		if (random < winChance) {
			...
			userData.money += winnings - amount;
	...
	slots: async function (message, args, userData, usersData, bank, bankData, senderID, fonts) {
		const amount = parseInt(args[1]);
		if (!amount || amount <= 0) {
...
`;
	...
	blackjack: async function (message, args, userData, usersData, bank, bankData, senderID, fonts) {
		const amount = parseInt(args[1]);
		if (!amount || amount <= 0) {
...
`;
	...
	roulette: async function (message, args, userData, usersData, bank, bankData, senderID, fonts) {
		const amount = parseInt(args[1]);
...
`;
	...
	lottery: async function (message, args, userData, usersData, bank, bankData, senderID, fonts) {
		const action = args[1]?.toLowerCase();
		...
		if (!action || action === "buy") {
			...
			if (userData.money < totalCost) {
				return message.reply(fonts.bold(`❌ Insufficient funds in wallet. Need $${totalCost.toLocaleString()}`));
			}
...
`;
	...
	leaderboard: async function (message, bankData, fonts, api, usersData) {
		try {
			const allBanks = await bankData.getAll();
			const richestUsers = Object.entries(allBanks)
				.filter(([uid, bank]) => bank && (bank.balance + bank.savings + bank.vault) > 0)
				.map(([uid, bank]) => ({
					uid,
			...
			const userInfoPromises = richestUsers.map(async (user) => {
				try {
					...
					user.name = userInfo[user.uid]?.name || `User ${user.uid.slice(0, 5)}`;
				} catch (error) {
					user.name = `User ${user.uid.slice(0, 5)}`;
				}
			});
	...
	rob: async function (message, args, bank, bankData, senderID, event, fonts, usersData) {
		const targetUID = Object.keys(event.mentions)[0];
